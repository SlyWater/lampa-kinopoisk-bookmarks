...
- Импорт полного списка из браузерной страницы Кинопоиска через userscript, если OAuth/Apps Script отдаёт только первую страницу.
...
- `tools/kinopoisk-folder-import.user.js` - userscript для браузерного импорта папки Кинопоиска в backend.
...
## Импорт полного списка через браузер

Если диагностика показывает `partialList: true` и `missingItemsCount` больше нуля, текущий OAuth/API-источник не отдал весь список. Для полного импорта используйте userscript:

1. Установите `tools/kinopoisk-folder-import.user.js` в Tampermonkey/Violentmonkey.
2. В Lampa откройте `Настройки -> Кинопоиск Закладки -> Импорт из браузера Кинопоиска`.
3. Запомните код импорта.
4. В браузере, где вы авторизованы в Кинопоиске, откройте `https://www.kinopoisk.ru/mykp/folders/3575/?format=mini`.
5. Нажмите кнопку `Lampa KP Import`, введите код и подтвердите отправку.
6. Вернитесь в Lampa и нажмите `OK` в окне импорта. После этого запустится обычная синхронизация и сборка TMDB-карточек.

Userscript отправляет на backend только список фильмов из DOM страницы: Kinopoisk ID, название, оригинальное название и дату добавления, если она есть на странице. Cookies, OAuth-токены и пароль не отправляются. На self-hosted backend импорт хранится в памяти процесса; после перезапуска сервера его нужно отправить заново, но уже собранные карточки сохраняются в локальном кэше Lampa.
