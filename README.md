...
При ручной синхронизации плагин запускает backend-job и показывает прогресс сборки карточек: сколько фильмов обработано, сколько карточек собрано и сколько не удалось сопоставить.
...
Backend собирает функциональные карточки по прямому `id_tmdb`; поиск по названию используется только как строгий fallback с проверкой года и названия. Сомнительные совпадения пропускаются и видны в диагностике как `unresolved`.
